...
| `/architecture` | **Shipped** | MVVM + repository (Flutter) and transport/logic/data (API) checked against the diff from `HEAD`; backend rules in `backends/*.md` |
...
| `/self-hosted-runner` | **Shipped** | Walk a local Mac: `config.sh`, LaunchAgent (not `svc.sh`), `pmset`, swap `runs-on` |
| `/firebase-setup` | **Shipped** | Checklist / Chrome: project, iOS app, RC `app_channel` condition |
| `/app-check` | **Shipped** | DeviceCheck / App Attest, debug tokens, backend enforcement flag |
| `/fastapi-setup` | **Shipped** | Expand the stub: uv, ruff, pytest, Docker optional |
| `/verify` | **Shipped** | Generic VM Service driver (tap / eval / screenshot) |
